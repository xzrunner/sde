#ifndef _IS_SDE_SPATIALINDEX_POINTER_POOL_QUADTREE_NODE_H_
#define _IS_SDE_SPATIALINDEX_POINTER_POOL_QUADTREE_NODE_H_
#include "Node.h"
using namespace IS_SDE::SpatialIndex;

namespace Tools
{
	template<> class PointerPool<QuadTree::Node>
	{
	public:
		...
		~PointerPool()
		{
			...
			while (! m_pool.empty())
			{
				QuadTree::Node* x = m_pool.top(); m_pool.pop();
#ifndef NDEBUG
				m_pointerCount--;
#endif
				delete x;
		...
		PoolPointer<QuadTree::Node> acquire()
		{
			if (! m_pool.empty())
			{
				QuadTree::Node* p = m_pool.top(); m_pool.pop();
#ifndef NDEBUG
				m_hits++;
#endif

				return PoolPointer<QuadTree::Node>(p, this);
			}
#ifndef NDEBUG
			else
			{
			...
			return PoolPointer<QuadTree::Node>();
		}

		void release(QuadTree::Node* p)
		{
			if (p != 0)
			{
	...
	protected:
		size_t m_capacity;
		std::stack<QuadTree::Node*> m_pool;
...
#endif // _IS_SDE_SPATIALINDEX_POINTER_POOL_QUADTREE_NODE_H_
