#ifndef _IS_SDE_SPATIALINDEX_POINTER_POOL_NVRTREE_NODE_H_
#define _IS_SDE_SPATIALINDEX_POINTER_POOL_NVRTREE_NODE_H_
#include "Node.h"
using namespace IS_SDE::SpatialIndex;

namespace Tools
{
	template<> class PointerPool<NVRTree::Node>
	{
	public:
		...
		~PointerPool()
		{
			...
			while (! m_pool.empty())
			{
				NVRTree::Node* x = m_pool.top(); m_pool.pop();
#ifndef NDEBUG
				m_pointerCount--;
#endif
				delete x;
		...
		PoolPointer<NVRTree::Node> acquire()
		{
			if (! m_pool.empty())
			{
				NVRTree::Node* p = m_pool.top(); m_pool.pop();
#ifndef NDEBUG
				m_hits++;
#endif

				return PoolPointer<NVRTree::Node>(p, this);
			}
#ifndef NDEBUG
			else
			{
			...
			return PoolPointer<NVRTree::Node>();
		}

		void release(NVRTree::Node* p)
		{
			if (p != 0)
			{
...
#endif // _IS_SDE_SPATIALINDEX_POINTER_POOL_NVRTREE_NODE_H_
