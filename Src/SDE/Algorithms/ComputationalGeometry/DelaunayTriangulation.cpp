...
DelaunayTriangulation::DelaunayTriangulation(const std::vector<MapPos2D>& src, bool bFixBound /*= true*/)
{
	if (src.size() >= 3)
	{
		initSurrondTri(src);
...
DelaunayTriangulation::~DelaunayTriangulation()
{
	...
	for_each(tris.begin(), tris.end(), Tools::DeletePointerFunctor<Triangle>());
	for_each(edges.begin(), edges.end(), Tools::DeletePointerFunctor<Edge>());
}

bool DelaunayTriangulation::insertNode(const MapPos2D& p)
{
	int type = Tools::SpatialMath::checkPosInTriangle(p, m_rootTri->m_nodes[0]->m_pos, m_rootTri->m_nodes[1]->m_pos, m_rootTri->m_nodes[2]->m_pos);
	if (type == -1 || type == 3)
		return false;
...
void DelaunayTriangulation::deleteNodes(const std::vector<MapPos2D>& pos)
{
...
bool DelaunayTriangulation::isBoundNode(const MapPos2D& n) const
{
	Node tmp(n);
...
void DelaunayTriangulation::getBoundLinePos(std::vector<std::vector<MapPos2D> >& bounds) const
{
	std::set<MapPos2D, PosDownCmp> leftPos;
	...
	while (!leftPos.empty())
	{
		...
		while (true)
		{
			...
			double minAngle = Tools::DOUBLE_MAX;
			MapPos2D nextPos;
			for (size_t i = 0; i < (*ptrCurrNode)->m_edges.size(); ++i)
			{
				...
				if (e->isMargin())
				{
					MapPos2D otherPos(e->m_nStart == *ptrCurrNode ? e->m_nEnd->m_pos : e->m_nStart->m_pos);
					...
					double angle;
					if (bound.size() < 2)
						angle = Tools::SpatialMath::getAnticlockwiseAngleStartXAxis(bound[bound.size() - 1], otherPos);
					else
						angle = Tools::SpatialMath::calAngle(bound[bound.size() - 2], bound[bound.size() - 1], otherPos);
					if (angle < minAngle)
					{
						minAngle = angle;
						nextPos = otherPos;
			...
			assert(minAngle != Tools::DOUBLE_MAX);
...
void DelaunayTriangulation::getSurroundPos(const MapPos2D& pos, std::vector<MapPos2D>& surround) const
{
	Node tmp(pos);
	std::set<Node*, NodeCmp>::const_iterator itr = m_nodes.find(&tmp);
	if (itr != m_nodes.end())
	...
	legalizeEdge(bool checkRightTri)
{
	...
	MapPos2D checkPos(m_tLeft.second->m_pos), fixPos(m_tRight.second->m_pos);
	if (checkRightTri)
		std::swap(checkPos, fixPos);

	MapPos2D center;
	Tools::SpatialMath::getCircumcenter(m_nStart->m_pos, m_nEnd->m_pos, fixPos, &center);
	if (Tools::SpatialMath::getDistanceWithSquare(checkPos, center) < Tools::SpatialMath::getDistanceWithSquare(fixPos, center))
	{
...
double DelaunayTriangulation::Edge::length() const
{
	return Tools::SpatialMath::getDistance(m_nStart->m_pos, m_nEnd->m_pos);
}
...
bool DelaunayTriangulation::insertNode(Node* n)
{
	...
	while (true)
	{
		if (curr->m_bLeaf)
		{
			int type = Tools::SpatialMath::checkPosInTriangle(n->m_pos, curr->m_nodes[0]->m_pos, curr->m_nodes[1]->m_pos, curr->m_nodes[2]->m_pos);
			switch (type)
			{
			case 0:
		...
		else
		{
			for (size_t i = 0; i < curr->m_children.size(); ++i)
			{
				...
				int type = Tools::SpatialMath::checkPosInTriangle(n->m_pos, child->m_nodes[0]->m_pos, child->m_nodes[1]->m_pos, child->m_nodes[2]->m_pos);
				if (type != -1)
				{
					curr = child;
					break;
...
void DelaunayTriangulation::deleteNode(const Node* n)
{
	...
	//double N = m_nodes.size(), K = totDel.size(), R = reinsertNodes.size();
	//fout << "N: " << N << '\t' << "K: " << K << '\t'
...
void DelaunayTriangulation::initSurrondTri(const std::vector<MapPos2D>& src)
{
	MapScope scope;
	Tools::SpatialMath::getMBR(src, scope);
	double extend = std::max(scope.xLength(), scope.yLength()) * BOUND_EXTEND_TIMES;
	MapPos2D center(0.5 * (scope.m_xMin + scope.m_xMax), 0.5 * (scope.m_yMin + scope.m_yMax));

	MapPos2D p0(center.d0, center.d1 + extend), 
		p1(center.d0 - extend, center.d1 - extend), 
		p2(center.d0 + extend, center.d1 - extend);
...
void DelaunayTriangulation::initSrcNodes(const std::vector<MapPos2D>& src)
{
...
void DelaunayTriangulation::buildTriangulationStruct()
{
	...
	Tools::Random::RandomPermutation(nodes);
...
{
	double threshold = getAvgEdgesLen() * BOUND_FIX_THRESHOLD_TIMES;
...
double DelaunayTriangulation::getAvgEdgesLen() const
{
	double totLen = 0;
	for (size_t i = 0; i < m_tris.size(); ++i)
	{
...
void DelaunayTriangulation::checkSingleNode()
{
	...
	for ( ; itr != m_nodes.end(); ++itr)
	{
		...
		if (bSingle)
		{
			...
			double minLen = Tools::DOUBLE_MAX;
			for (size_t i = 0; i < (*itr)->m_edges.size(); ++i)
			{
				...
				{
					MapPos2D other = e->leftNode()->m_pos;
					double maxLenEdge = std::max(
						Tools::SpatialMath::getDistance(e->m_nStart->m_pos, other),
						Tools::SpatialMath::getDistance(e->m_nEnd->m_pos, other)
						);
				...
				if (!tri->isFake() && tri->m_bRemoved)
				{
					MapPos2D other = e->rightNode()->m_pos;
					double maxLenEdge = std::max(
						Tools::SpatialMath::getDistance(e->m_nStart->m_pos, other),
						Tools::SpatialMath::getDistance(e->m_nEnd->m_pos, other)
						);
