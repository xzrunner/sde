...
IS_SDE::IStorageManager* IS_SDE::StorageManager::returnDiskStorageManager(Tools::PropertySet& ps)
{
	IStorageManager* sm = new DiskStorageManager(ps);
	return sm;
}

IS_SDE::IStorageManager* IS_SDE::StorageManager::createNewDiskStorageManager(const std::wstring& baseName, size_t pageSize)
{
	Tools::Variant var;
...
IS_SDE::IStorageManager* IS_SDE::StorageManager::loadDiskStorageManager(const std::wstring& baseName)
{
	Tools::Variant var;
	Tools::PropertySet ps;
	...
	: m_pageSize(0), m_nextPage(-1), m_buffer(NULL)
{
	...
	if (var.m_varType != Tools::VT_EMPTY)
	{
		if (var.m_varType != Tools::VT_PWCHAR)
			throw Tools::IllegalArgumentException("SpatialDataBase::DiskStorageManager: Property FileName must be Tools::VT_PWCHAR");
		...
		if (bFileExists == true && bOverwrite == false)
		{
			...
			if (m_indexFile.fail() || m_dataFile.fail())
				throw Tools::IllegalArgumentException("SpatialDataBase::DiskStorageManager: Index/Data file cannot be read/writen.");
		}
		else
		{
			...
			if (m_indexFile.fail() || m_dataFile.fail())
				throw Tools::IllegalArgumentException("SpatialDataBase::DiskStorageManager: Index/Data file cannot be created.");

		}
	}
	else
	{
		throw Tools::IllegalArgumentException("SpatialDataBase::DiskStorageManager: Property FileName was not specified.");
	}
	...
	if (bOverwrite == true)
	{
		...
		if (var.m_varType != Tools::VT_EMPTY)
		{
			if (var.m_varType != Tools::VT_ULONG)
				throw Tools::IllegalArgumentException("SpatialDataBase::DiskStorageManager: Property PageSize must be Tools::VT_ULONG");
			m_pageSize = var.m_val.ulVal;
			m_nextPage = 0;
		}
		else
		{
			throw Tools::IllegalArgumentException("SpatialDataBase::DiskStorageManager: A new storage manager is created and property PageSize was not specified.");
		}
	}
	else
	{
		...
		if (m_indexFile.fail())
			throw Tools::IllegalStateException("SpatialDataBase::DiskStorageManager: Failed reading pageSize.");

		m_indexFile.read(reinterpret_cast<char*>(&m_nextPage), sizeof(id_type));
		if (m_indexFile.fail())
			throw Tools::IllegalStateException("SpatialDataBase::DiskStorageManager: Failed reading nextPage.");
	}
	...
	if (bOverwrite == false)
	{
		...
		if (m_indexFile.fail())
			throw Tools::IllegalStateException("SpatialDataBase::DiskStorageManager: Corrupted storage manager index file.");
...
void DiskStorageManager::loadByteArray(const id_type id, size_t& len, byte** data)
{
	...
	*data = new byte[len];
	...
	do
	{
		...
		if (m_dataFile.fail())
			throw Tools::IllegalStateException("SpatialDataBase::DiskStorageManager: Corrupted data file.");
...
//void DiskStorageManager::getNodePageInfo(id_type id, size_t& length, size_t& pageSize) const
//{
