...
bool SpatialMath::isPointInRect(double x, double y, double x1, double y1, double x2, double y2, bool bTouch/* = true*/)
{
	...
	if (bTouch)
	{
		return x >= l - Tools::DOUBLE_TOLERANCE && x <= r + Tools::DOUBLE_TOLERANCE
			&& y >= b - Tools::DOUBLE_TOLERANCE && y <= t + Tools::DOUBLE_TOLERANCE;	
	}
	else
	{
		return x > l + Tools::DOUBLE_TOLERANCE && x < r - Tools::DOUBLE_TOLERANCE
...
{
	return (p.d1 - s.d1) * (e.d0 - s.d0) - (p.d0 - s.d0) * (e.d1 - s.d1) > Tools::DOUBLE_TOLERANCE;
}
...
{
	double squareLenAC = getDistanceWithSquare(a, center),
		squareLenBC = getDistanceWithSquare(b, center),
		squareLenAB = getDistanceWithSquare(a, b);

	return squareLenAC + squareLenBC - squareLenAB > 0;
}

bool SpatialMath::isTurnLeft(const MapPos2D& a, const MapPos2D& center, const MapPos2D& b)
{
	return (center.d0 - a.d0) * (b.d1 - center.d1) - (b.d0 - center.d0) * (center.d1 - a.d1) > 0;
}

bool SpatialMath::isTurnRight(const MapPos2D& a, const MapPos2D& center, const MapPos2D& b)
{
	return (center.d0 - a.d0) * (b.d1 - center.d1) - (b.d0 - center.d0) * (center.d1 - a.d1) < 0;
}
...
double SpatialMath::getAnticlockwiseAngleStartXAxis(const MapPos2D& s, const MapPos2D& e)
{
	if (isVerticalLine(s, e))
	{
		if (s.d1 == e.d1)
			return 0;
		else if (s.d1 > e.d1)
			return Tools::SPATIAL_MATH_PI * 1.5;
		else
			return Tools::SPATIAL_MATH_PI * 0.5;
	}
	else
	{
		if (s.d1 == e.d1)
			return s.d0 < e.d0 ? 0 : Tools::SPATIAL_MATH_PI;
		else
		{
			double ret = atan((s.d1 - e.d1) / (s.d0 - e.d0));
			if (ret >= 0)
				return s.d1 > e.d1 ? ret + Tools::SPATIAL_MATH_PI : ret;
			else
				return s.d1 > e.d1 ? 2 * Tools::SPATIAL_MATH_PI + ret : Tools::SPATIAL_MATH_PI + ret;
		}
	}
}

double SpatialMath::calPolygonArea(const std::vector<MapPos2D>& poly)
{
	if(poly.size() < 3)
		return 0.0;
	...
	for(size_t i = 1; i < size ; i++)
		s += (poly[i].d1 + poly[i - 1].d1) * (poly[i].d0 - poly[i-1].d0);		
	
	if(poly[0] != poly[size - 1])
		s += (poly[0].d1 + poly[size - 1].d1) * (poly[0].d0 - poly[size -1].d0);

	return fabs(s/2.0);
}

double SpatialMath::calAngle(const MapPos2D& a, const MapPos2D& center, const MapPos2D& b)
{
	double lenAC = getDistance(a, center),
		lenBC = getDistance(b, center),
		lenAB = getDistance(a, b);

	double cosVal = (lenAC * lenAC + lenBC * lenBC - lenAB * lenAB) / (2 * lenAC * lenBC);
	double angle = acos(cosVal);

	if (isTurnRight(a, center, b))
		return angle;
	else
		return 2 * Tools::SPATIAL_MATH_PI - angle;
}
									   ...
									   const double tolerance/* = DOUBLE_ZERO*/)
{
	...
	if (fabs(denominatorX) < Tools::DOUBLE_TOLERANCE || fabs(denominatorY) < Tools::DOUBLE_TOLERANCE)
	{
		crossPoint->d0 = Tools::DOUBLE_MAX;
		crossPoint->d1 = Tools::DOUBLE_MAX;
		return false;
	}
												  ...
												  const double tolerance/* = DOUBLE_ZERO*/)
{
	...
	assert (fabs(denominatorX) > Tools::DOUBLE_TOLERANCE && fabs(denominatorY) > Tools::DOUBLE_TOLERANCE);
	crossPoint->d0 = ( (e0.d0 * s0.d1 - s0.d0 * e0.d1) * (e1.d0 - s1.d0) - (e1.d0 * s1.d1 - s1.d0 * e1.d1) * (e0.d0 - s0.d0) ) / denominatorX;
	crossPoint->d1 = ( (e0.d1 * s0.d0 - s0.d1 * e0.d0) * (e1.d1 - s1.d1) - (e1.d1 * s1.d0 - s1.d1 * e1.d0) * (e0.d1 - s0.d1) ) / denominatorY;
	return true;
}
...
int SpatialMath::getFootOfPerpendicular(const MapPos2D& s, const MapPos2D& e, const MapPos2D& out, MapPos2D* foot)
{
	...
	if (dxSquare + dySquare < Tools::DOUBLE_TOLERANCE)
	{
		*foot = s;
		return -1;
	}
...
void SpatialMath::getPartCtlposOfLine(double s, double e, const std::vector<MapPos2D>& tot, std::vector<MapPos2D>* part)
{
	...
	size_t sIndex = static_cast<size_t>(ceil(s));
	while (sIndex < e)
...
void SpatialMath::getPosOnLineByPointPosition(const std::vector<MapPos2D>& l, double position, MapPos2D* p)
{
	...
	size_t index = static_cast<size_t>(floor(position));
...
void SpatialMath::getRectChildGrid(const Rect& src, size_t position, Rect& dest)
{
...
size_t SpatialMath::getPositionPosInRect(const Rect& parent, const MapPos2D& p)
{
...
void SpatialMath::getTriangleCircumcenter(const std::vector<MapPos2D>& tri, MapPos2D* circumcenter)
{
	assert(tri.size() == 3);
	bool bHor01 = isHorizontalLine(tri[0], tri[1]),
		bHor12 = isHorizontalLine(tri[1], tri[2]);
	assert(!bHor01 || !bHor12);

	double cx01 = 0.5 * (tri[0].d0 + tri[1].d0),
		cx12 = 0.5 * (tri[1].d0 + tri[2].d0);
	if (bHor01)
	{
		circumcenter->d0 = cx01;
		circumcenter->d1 = findYOnSeg(tri[1], tri[2], cx01);
	}
	else if (bHor12)
	{
		circumcenter->d0 = cx12;
		circumcenter->d1 = findYOnSeg(tri[0], tri[1], cx12);
	}
	else
	{
		MapPos2D c01(cx01, 0.5 * (tri[0].d1 + tri[1].d1)), 
			c12(cx12, 0.5 * (tri[1].d1 + tri[2].d1));
		double kp01 = - 1 / getLineSlope(tri[0], tri[1]),
			kp12 = - 1 / getLineSlope(tri[1], tri[2]),
			bp01 = c01.d1 - kp01 * c01.d0,
			bp12 = c12.d1 - kp12 * c12.d0;

		assert(fabs(kp01 - kp12) > Tools::DOUBLE_TOLERANCE);
		circumcenter->d0 = (bp12 - bp01) / (kp01 - kp12);
		circumcenter->d1 = kp01 * circumcenter->d0 + bp01;
	}
}