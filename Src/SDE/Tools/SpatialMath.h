#ifndef _IS_SDE_TOOLS_SPATIAL_MATH_H_
#define _IS_SDE_TOOLS_SPATIAL_MATH_H_
#include "Tools.h"
...
namespace Tools
{
	...
	public:
		...
		static bool isBetween(double bound0, double bound1, double test) {
			if (bound0 < bound1)
				return test < bound1 + Tools::DOUBLE_TOLERANCE && test > bound0 - Tools::DOUBLE_TOLERANCE;
			else
				return test < bound0 + Tools::DOUBLE_TOLERANCE && test > bound1 - Tools::DOUBLE_TOLERANCE;
		}
		...
		static bool isHorizontalLine(const MapPos2D& p0, const MapPos2D& p1) {
			return fabs(p0.d1 - p1.d1) < Tools::DOUBLE_TOLERANCE;
		}
		static bool isVerticalLine(const MapPos2D& p0, const MapPos2D& p1) {
			return fabs(p0.d0 - p1.d0) < Tools::DOUBLE_TOLERANCE;
		}

		// To check angle a-center-b turn left or right.
		static bool isTurnLeft(const MapPos2D& a, const MapPos2D& center, const MapPos2D& b);
		static bool isTurnRight(const MapPos2D& a, const MapPos2D& center, const MapPos2D& b);
		...
		// Get the angle of the line from s to e
		// The angle is anticlockwise, start from X positive axis and range from 0 to 2*pi
		static double getAnticlockwiseAngleStartXAxis(const MapPos2D& s, const MapPos2D& e);

		// Compute the area of polygon
		static double calPolygonArea(const std::vector<MapPos2D>&v_pos);

		// Compute the angle a-center-b, from a to b with anticlockwise
		// return range from 0 to 2*pi
		static double calAngle(const MapPos2D& a, const MapPos2D& center, const MapPos2D& b);
		...
		// 0 1
		// 2 3
		static void getRectChildGrid(const Rect& src, size_t position, Rect& dest);

		// Using for QuadTree, position from 0 to 3, indicate the position of the child
		// Return the position of point in Rect.
		// 0 1
		// 2 3
		static size_t getPositionPosInRect(const Rect& parent, const MapPos2D& p);

		// Get the intersection of edges' midperpendicular
		// Also it is the center of tri's circumcircle
		static void getTriangleCircumcenter(const std::vector<MapPos2D>& tri, MapPos2D* circumcenter);

		static double getLineSlope(const MapPos2D& p0, const MapPos2D& p1) {
			assert(!isHorizontalLine(p0, p1));
			return (p0.d1 - p1.d1) / (p0.d0 - p1.d0);
		}
	};

}

#endif // _IS_SDE_TOOLS_SPATIAL_MATH_H_