...
public:
	...
	IS_GIS::Pos3D project6DuZone(IS_GIS::Pos3D& posSource );	//6度分带投影

	IS_GIS::Pos3D inverse6DuZoneToGeographic(IS_GIS::Pos3D& posSource );//反算6度分带地理坐标

	IS_GIS::Pos3D project3DuZone(IS_GIS::Pos3D& posSource );	//3度分带投影

	IS_GIS::Pos3D inverse3DuZoneToGeographic(IS_GIS::Pos3D& posSource );//反算3度分带地理坐标

	IS_GIS::Pos3D projectFromGeographic( const IS_GIS::Pos3D& posSource) const; //投影到同一个坐标

	IS_GIS::Pos3D inverseToGeographic( const IS_GIS::Pos3D& posSource) const;   //反算回地理坐标
	...
	IS_GIS::Pos3D getDXYFromGeography(double x80, double y80, const IS_GIS::Pos3D& shift_val, const IS_GIS::Pos3D& posSource) const; // 返回结果（DX，DY，0）

	//IS_GIS::Pos3D selZoneProject(int zone, IS_GIS::Pos3D& posSource); //换到第zone带投影

	//double countDistance(IS_GIS::Pos3D& posBegin, IS_GIS::Pos3D& posEnd); //测量高斯投影上两点的距离 
